...
class TravelTasks():
  ...
  def gather_task(self, agent, origin, cities, interests, range, hints):
    return Task(description=dedent(f"""
        ...
        high level costs.
        Additional to the main sights and attractions, also
        recommend places and activities that are off the beaten track
        and usually only know to locals. That give intimate insights
        into the local's lifestyle.
  ...
  def review_task(self, agent, origin, interests, range):
    return Task(description=dedent(f"""
        Review the proposed travel plan. Double check if the proposed
        locations and activities are really good. Check reviews of other
        travellers on websites like booking.com, google maps, tripadvisor.com
        If the plan has weaknesses, give feedback to the concierge or local expert.
        
        Your final answer MUST be a complete reviewed travel plan,
        formatted as markdown, encompassing a daily schedule,
        anticipated weather conditions, recommended clothing and
        items to pack, and a detailed budget, ensuring THE BEST
        TRIP EVER, Be specific and give it a reason why you picked
